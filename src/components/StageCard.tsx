...
export default function StageCard({ stage, bestTime, onSelect }: StageCardProps) {
  return (
    <motion.div
      ...
      aria-label={`${stage.label}, ${stage.size}×${stage.size} grid${bestTime !== undefined ? `, best time ${formatTime(bestTime)}` : ''}`}
    ...
    >
      <div className="flex flex-col gap-0.5">
        <span className="font-bold text-gray-800 text-base">{stage.label}</span>
